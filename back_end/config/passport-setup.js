    ...
    //else {
        ...
        User.findOne({email: email}, (err,user)=> {
            ...
            if(user) {
                ...
                return done(null,user,req.flash('error','Email already exists'))
            }
            if (!user) {
...
passport.use('login', new localStrategy({
    ...
    User.findOne({email: email}, (err,user)=> {
        ...
        if (err) {
            return done(err,false, req.flash('error', 'Error...'))
        } 
        if(!user) {
        ...
        if (user) {
            ...
            if (user.validatePassword(password, user.password)) {
                ...
                return done(null,user, req.flash('error', 'please check your password'))
